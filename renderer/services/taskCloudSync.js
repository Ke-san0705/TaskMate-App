...
function readKnownKeys(userId) {
  try {
    const parsed = JSON.parse(localStorage.getItem(knownItemsKey(userId)) || '[]');
...
function writeKnownKeys(userId, keys) {
  localStorage.setItem(knownItemsKey(userId), JSON.stringify([...keys].sort()));
}
...
async function readLocalItems() {
  const [taskResult, projectState] = await Promise.all([
    window.taskMate.getTasks(),
    window.taskMate.getProjectState()
  ]);
  const items = [];
...
async function applyLocalUpsert(item) {
  ...
  if (item.itemType === ITEM_TYPES.TASK) {
    ...
    return exists
      ? window.taskMate.updateTask(item.localId, payload)
      : window.taskMate.addTask(payload);
  }
  if (item.itemType === ITEM_TYPES.CATEGORY) {
    const exists = state.categories.some((category) => category.id === item.localId);
    return exists
      ? window.taskMate.updateProjectCategory(item.localId, payload)
      : window.taskMate.createProjectCategory(payload);
  }
  if (item.itemType === ITEM_TYPES.PROJECT) {
    ...
    return exists
      ? window.taskMate.updateProject(item.localId, input)
      : window.taskMate.createProject(input);
  }
  if (item.itemType === ITEM_TYPES.MILESTONE) {
    ...
    return exists
      ? window.taskMate.updateProjectMilestone(item.localId, payload)
      : window.taskMate.createProjectMilestone(payload);
  }
  if (item.itemType === ITEM_TYPES.PROJECT_TASK) {
    ...
    return exists
      ? window.taskMate.updateProjectTask(item.localId, payload)
      : window.taskMate.createProjectTask(payload);
  }
  return null;
}
...
export async function syncTaskData() {
  ...
  const finalKnownKeys = new Set([
    ...knownKeys,
    ...finalMap.keys(),
    ...remoteDeletedKeys
  ]);
  writeKnownKeys(user.id, finalKnownKeys);
  await Promise.all([
    window.taskMate.reloadTasks(),
    window.taskMate.getProjectState()
  ]);
