...
function registerLaunch(state, now = new Date()) {
  ...
  // 再訪演出は「同じ日に何度も起動しても繰り返さない」ことが体験上重要です。
  // そのため、lastReconnectDateで当日表示済みかを確認してからイベント化します。
...
function markCelebrated(state, dateKey = localDateKey()) {
  const next = normalizeLifeState(state);
