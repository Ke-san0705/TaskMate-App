...
- Windows 11
- Windows 10でも動作する可能性はありますが、主な確認対象はWindows 11です
...
TaskMateのタスクデータや設定は、利用者のPC内に保存されます。パッケージ版では、ユーザーが変更するJSONデータをElectronの`userData`配下に保存し、アプリ本体のインストールフォルダへは書き込みません。

現在のPC版TaskMateでは、タスク内容を外部サーバーへ送信しません。外部通信の確認では、実アプリ側の送信処理は見つかっておらず、検出された`fetch`は開発時のブラウザモックがローカルJSONを読む用途です。インターネット接続なしでも基本機能を利用できます。

## 開発者向け

Node.js 22以上とnpm 10以上を使用します。
...
Windows向けInstallerは次で作成します。

```powershell
npm run dist
```

PowerShellの実行ポリシーで`npm.ps1`がブロックされる場合:
...
初回起動時、Windows Defender SmartScreenの警告が表示される場合があります。これは個人開発アプリでコード署名が未設定の場合に起こることがあります。今後、利用者が増えてきた段階でコード署名と自動更新の導入を検討します。
