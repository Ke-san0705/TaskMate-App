...
test('時刻なしタスクはローカル時刻で夕方以降に状態が上がる', () => {
  assert.equal(
    classifyTask(task({ time: null }), new Date(2026, 5, 18, 16, 30)).state,
    'calm'
  );
  assert.equal(
    classifyTask(task({ time: null }), new Date(2026, 5, 18, 17, 0)).state,
    'warning'
  );
  assert.equal(
    classifyTask(task({ time: null }), new Date(2026, 5, 18, 21, 0)).state,
    'urgent'
  );
  assert.equal(
    classifyTask(task({ date: '2026-06-17', time: null }), new Date(2026, 5, 18, 9, 0)).state,
    'overdue'
  );
});

test('未来タスクはfutureを基本にする', () => {
  const now = new Date(2026, 5, 18, 10, 0, 0);
  assert.equal(classifyTask(task({ date: '2026-06-20', time: '18:00' }), now).state, 'future');
  assert.equal(classifyTask(task({ date: '2026-06-20', time: null }), now).state, 'future');
});
...
test('タスク0件の生活圧は0', () => {
  const result = aggregateLifePressure([], new Date(2026, 5, 18, 10, 0));
