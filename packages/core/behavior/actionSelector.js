...
function selectAction({
  ...
  const movementDisabled =
    ...
    normalizedSettings.reduceMotion === true ||
    reducedMotion ||
    level === LIFE_PRESSURE_LEVELS.OVERLOADED ||
    movementRecentlyUsed(lifeState, now);
  if (movementDisabled) {
    // スマホでは「忙しいほど激しく動く」より、動きを減らして一つへ絞る方が体験に合います。
  ...
  return {
    ...
    movement: movementFor(selected.movement, normalizedSettings, random),
    ambientLevel: AMBIENT_LEVEL_BY_PRESSURE[level] || 0,
