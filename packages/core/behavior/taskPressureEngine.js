...
function localDateKey(date = new Date()) {
  // toISOString()はUTC日付になるため、日本時間など端末ローカルの「今日」とずれます。
  // タスクの日付判定ではユーザーの端末カレンダーを正とするので、ローカルの年月日から組み立てます。
...
function classifyTimedTask(task, now) {
  ...
  // highタスクは、ユーザーが早めに気づけるよう通常より少し早くwarning/urgentへ上げます。
  // ただし明日以降の通常タスクまで全部ホームを圧迫しないよう、優先度で段階を分けます。
...
function aggregateLifePressure(tasks = [], now = new Date()) {
  ...
  // 生活圧は「最大の危険度」を強めに見ます。多数の低圧タスクがあっても、
  // 1件の急ぎタスクが埋もれないようにするためです。
  const score = clamp(
